...
* **标题吸顶（多级）+ 同级折叠**——文档内容不被修改
* **表格换行两档**（`epytor.tableWrapMode`：`wrap` / `nowrap`）+ 单元格内 Shift+Enter 软换行
* **Frontmatter 可视面板**：直接编辑 key/value 行；列表、注释与空行原样保留
* **工具栏溢出菜单**：窗口收窄时按钮收进「⋯」
* **Mermaid 预览缩放**（0.2×–3×，可重置，横向滚动）
* **加载指示**：打开或切回 Markdown 时显示（快速加载不会闪出）

### 体验

* **打开或切回 Markdown 标签不再闪动**——不再出现「先出现、又消失、再出现」
* **切回标签恢复滚动位置与折叠的标题**
* **吸顶切换时机与内置编辑器一致**：最多三级，碰到即固定
* **目录面板开合后，吸顶标题与工具栏立即重排**
...
* **切回标签后撤销/重做历史从零开始**——内容、滚动位置与折叠状态会恢复，撤销栈不会
...
* **「编辑器无响应，文件可能以过期内容保存」不再误报**——此前什么都没做、只是开着几个 Markdown 文件也会出现
* **切走标签不再丢刚输入的内容**，切回后看到的就是最后输入的样子
* **外部写盘不再被自动保存覆盖**——脚本或 AI 改动的文件保留其内容
* **开着 Markdown 页面时整个 VS Code 输入卡顿、切换 webview 时整窗口闪动**已消失
* **资源管理器点 Markdown 文件时标签狂闪**、切文档后无法输入（焦点互抢）已修复
* **源码 ↔ 预览**：光标不再回到 1 行 1 列或定位到错误块、不再产生重复标签、不再停在文件开头、大文件切回不再丢位置
* **吸顶标题**：不再整条消失，点击可定位（含第一个标题）
* **目录**：跳转后标题不再被吸顶条遮挡、不再残留吸顶条，折叠状态不再被同名标题或其它文档共享
* **打开文档不再被滚动到第一个标题**（frontmatter 被顶出视口）
* **表格单元格内 Shift+Enter 不再把内容变成普通段落**，软换行保存/重开往返一致
...
* **改了序列化模式/调试开关后，一次外部写盘不再把它改回旧值**
* **查找栏**：关闭后不再执行上一次搜索；无效正则给出提示
* **路径与语言补全**：键盘导航不再丢失高亮
* **代码块保持语法高亮**
* **行内代码块尾编辑**：块尾输入保持代码样式（`./` `@/` 路径补全可用）；按 → 退出并显示块内/块外指示
* **切换文件后焦点恢复**：切回页面自动恢复编辑器焦点（此前光标在但输入无效）
* **未编辑不再出现未保存圆点**：首帧与仅移动光标都不再误标脏
* **状态栏字数正常更新**，引用块内的标题会出现在目录里
* **Mermaid 缩放基准**：以原始渲染尺寸为基准（此前相对容器宽度，默认反而放大）
* **安全**：外部链接仅允许 http/https/mailto；工作区内的图片路径与路径链接不再能越出工作区；上传错误不再回显服务器响应体
