...
* **Word 式多级列表标记**：编号按层级显示为 `1.` / `a)` / `i.`，圆点显示为 ● / ■ / ◆，第四层回到第一档（源码与保存结果不变，仍是 `1.` / `-`）
...
* **大文档输入卡顿**（#16，感谢 @Jurhoo 的报告与初步定位）：约 3000 行内保持流畅（超大规模见已知限制）。按其定位方向，把「每次编辑都对整篇文档做全文差异计算」改为按行锚点的局部改动；本版自身还做了：输入期间不在后台做整篇处理（停手后才交接给 VS Code）、标题吸顶缓存改为文档坐标 + 防抖（滚动时零 DOM 查询）、序列化改为拉取式（输入期间零序列化）、撤销粒度固定为每次输入一步
