...
describe("computeStickyActiveIndex", () => {
    it("跳转场景：前一个标题滚出、目标标题完整可见 应该 隐藏（回归：点击吸顶条跳转后被前一个标题遮挡）", () => {
        const headings = [
            { top: -20, bottom: 10 }, // 前一章节：完全在顶栏上方
            { top: 44, bottom: 80 }, // 目标标题：完整可见（top >= 顶栏底）
        ];
        expect(computeStickyActiveIndex(headings, TOP)).toBe(-1);
    });

    it("正常滚动：前一个标题滚出且目标标题被顶栏遮挡 应该 吸顶前一个", () => {
        const headings = [
            { top: -60, bottom: -20 }, // 已滚出
            { top: 20, bottom: 60 }, // 目标标题顶部被顶栏遮住（top < 顶栏底）
        ];
