 ...
 * 不画（第 5 条，正文照常显示）。
 *
 * **真机复测（2026-09-11，VS Code 1.137.0）——两条结论推翻了上面这套因果**：
 *   1. 本版宿主**不摘挂 iframe**：3 轮切走/切回期间本模块只收到 resize 事件 **0 次**、
 *      记到的视口最小仍是 **1047×712**、折叠判定命中 **0 次**。即折叠守卫在本版宿主上
 *      永不生效，只为仍会摘挂的旧宿主（1.136）保留。
 *   2. 折叠期内层画的东西**不上屏**：把折叠期整个视口涂成品红后逐帧回放，一帧都找不到。
 *      用户看到的是宿主重新上屏 webview 之前的**空画面**（稳态 1–2 帧，约 10–40ms），
 *      与「折叠期该画什么」无关——所以历次围绕「空白 / 裁切 / 冻版式」的取舍都不改变
 *      屏幕结果。
 * 证据与复现入口：`_poc/TEMP/vscode-flash-probe.mjs`（外层 screencast + 内层读数），
 * 台账见 docs/tech-debt.md 的「折叠期守卫真机复测」条。
 *
 * 本模块只做三件事，都不改变正文版式：
