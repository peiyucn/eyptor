...
* [ ] **非激活面板的保存不拉取（`_panelActive` 闸门，2026-09-11 清账登记）** — 保活下非激活面板其实还活着、多半能回包，但 `_requestContent` 仍按保守口径短路，用 `_pushedContent`（最陈旧 400ms）或扩展内存内容落盘。README「停手后约 400ms 内切走标签：切标签触发的那次保存可能漏掉最后几个字符（编辑器回推的内容副本是停手后才生成的）。这部分改动会在下一次编辑或保存时写入；在那之前关掉窗口就会丢」正来自这条路径（失活落盘在置 `_panelActive=false` 之后触发；blur/pagehide 的补推只写 `_pushedContent` 与 `_webviewDirty`，不置文档脏标记，所以不会再触发一次自动保存，关标签时 `onDidDispose` 又直接删掉副本）。放开闸门能否消除该窗口需真机验证：切标签 → 等自动保存 → 核对文件内容与 webview 内容是否一致；未验证前不改行为（回归 a4349cc 曾因等待不存在/未就绪的 webview 报「编辑器无响应」）
* [ ] **保存保真：剩余四条「用户原文被改写」**（2026-09-13 核对 README 时用真实保存路径 `getMarkdownForSave()` 逐项实测）— 列表标记符与嵌套缩进已在本轮修掉（`detectListMarkerStyle` + minimalDiff 行签名归一化，回归用例 `webview/__tests__/saveFidelity.test.ts`），以下四条未处理，共同点仍是「语义变了或写法被改写」：**① 混合标记符文件被拆成多个列表并插入空行**——`- a\n* b\n- c` 存成 `- a\n\n* b\n\n- c`（remark 把标记符变化当成新列表，mdast 在相邻块之间补空行；单一标记符的文件不受影响）；**② `~x~` 被写成 `~~x~~`**——`H~2~O and x^2^` 存成 `H~~2~~O and x^2^`，源头是 remark-gfm 的 `singleTilde` 默认为 true（`~2~` 被解析成删除线，写回按标准删除线写法），**给文件加了新语义**；按 GFM 规范走可在 `remarkGFMPlugin.options` 关掉 `singleTilde`；**③ GitHub 提示块被转义**——`> [!NOTE]` 存成 `> \[!NOTE]`，GitHub 上不再渲染成提示块；**④ 以列表/表格结尾的文档 编辑后末尾多一个空行**——上游 `@milkdown/kit/plugin/trailing` 会在文档末尾补一个空段落（为了光标能落到最后一个块之后），它被序列化成末尾空行：`- one\n- two\n` 编辑后存成 `- one!\n- two\n\n`（未编辑时不补）。处理口径需 owner 定：②③ 涉及 clean 模式放宽转义的边界（放宽会引入 `[` / `~` 的结构歧义风险），④ 取决于「末尾空段落」要不要参与落盘
