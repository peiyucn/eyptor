...
* [ ] **非激活面板的保存不拉取（`_panelActive` 闸门，2026-09-11 清账登记）** — 保活下非激活面板其实还活着、多半能回包，但 `_requestContent` 仍按保守口径短路，用 `_pushedContent`（最陈旧 400ms）或扩展内存内容落盘；失活落盘（`_registerViewStateHandler` 里置 `_panelActive=false` 之后的保存）正走这条路径。**owner 真机手测（2026-09-13）：停手后立刻切标签，最后一次改动仍写进了文件，「400ms 窗口」未复现** —— 与代码里那句「blur 补推与宿主触发保存的先后没有保证」并存，即：路径上存在理论窗口、真机上未被观测到。README 已按 owner 决定删掉该条；本项保留的理由只剩「闸门是保守口径」这一条，放开闸门能否更稳仍需真机验证：切标签 → 等自动保存 → 核对文件内容与 webview 内容是否一致；未验证前不改行为（回归 a4349cc 曾因等待不存在/未就绪的 webview 报「编辑器无响应」）
