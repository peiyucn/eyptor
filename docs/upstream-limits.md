...
> 最近验证：2026-09-13（Milkdown 7.22.1 / VS Code 1.137）——第 1、2、3 项对应的上游 issue 经 `gh` 复核**全部 open**（#2413 / #2414 / #2415 / #2475）；第 1、3、5 项 epytor 侧已各自解决（见「追踪」列）；第 4 项是宿主行为，读数见下。
...
| 5 | 复选框后不写内容的空任务项（`- [ ] ` / `- [x]`，标记后无内容）识别不了复选框：`checked` 停在 null、`[ ]` 留成正文文本（显示为「• [ ]」），写回时 `[` 还被转义成 `- \[ ]` | ✘（**epytor 侧已解决**） | Milkdown / remark 任务列表解析（micromark 的分词器要求标记后有内容） | 上游未提 issue（2026-09-13 先自行修复） — **已解决（2026-09-13）**：解析后在 mdast 上补齐（`webview/utils/emptyTaskListItem.ts`：列表项首块是「只含 `[ ]` / `[x]` 标记的段落」时按标记设 `checked`、摘掉标记文本），序列化侧再用 `markdownSerializer.stripListItemBreakPlaceholder` 擦掉空段落的 `<br />` 占位（占位在 → 写进用户文件；占位提前吞掉 → 复选框标记一起消失），于是 `- [ ] ` 既能显示成复选框、存盘又逐字不变。回归用例：`webview/__tests__/emptyTaskListItem.test.ts`、`saveFidelity.test.ts`。**升级 `@milkdown/*` 时必须复验**：上游分词器一旦支持「标记后无内容」，本修复会自动让位（`checked` 已是布尔值时跳过），届时整层删掉 |
