...
* **列表**：有序 / 无序 / 任务列表（含 `- [ ] ` 这种标记后不写内容的空复选框）；标记按层级显示（`1.` / `a)` / `i.` 与 ● / ■ / ◆，第四层回绕）；列表项行首 Backspace 断开列表、把该项变成普通行；保存保留你写的标记符（`-` / `*`、`1.` / `1)`）与缩进
...
* **图片**：粘贴 / 拖放 / 图库选择插入；拖边缩放、可写图注；加载失败有提示可重试
...
| `epytor.imageLocalPath` | `""` | 本地图片目录（相对工作区根目录或 Markdown 文件，也可绝对路径）。留空则自动检测 `images/`、`imgs/`、`assets/images/`、`assets/`；都没有时用文件同级的 `images/` |
...
**图片**：`epytor.imageStorage` 为 `server` 时图片上传到你的图床（必须先填 `epytor.imageServer.url`，否则上传失败并说明原因）；为 `local` 时存到 `epytor.imageLocalPath` 或自动检测到的目录。各字段含义见设置面板。安全上：工作区级 `.vscode/settings.json` 既不能把 `epytor.imageLocalPath` 指到工作区外，也不能替你把上传打开——两种情况下图片都存本地并给出提示。
