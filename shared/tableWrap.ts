...
/**
 * 三档语义：
 * - normal：CJK 不拆词、长词折行（word-break: keep-all）
 * - aggressive：任意字符断行（word-break: break-all）
 * - none：不换行 + 横向滚动（white-space: nowrap + overflow-x: auto）
 * 未知值安全回退到 normal。
 */
export function resolveTableWrapVars(mode: string): TableWrapVars {
    switch (mode) {
        case "aggressive":
            return { wordBreak: "break-all", whiteSpace: "normal", overflowX: "visible" };
        case "none":
            return { wordBreak: "keep-all", whiteSpace: "nowrap", overflowX: "auto" };
        default:
            return { wordBreak: "keep-all", whiteSpace: "normal", overflowX: "visible" };
    }
}
